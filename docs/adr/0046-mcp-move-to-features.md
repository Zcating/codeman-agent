...
- **Supersedes**: 无 — 纯结构性 move,与 互补(ADR-0041 拆关注点,本 ADR 统一目录约定)
- **Related**:
  -  — MCP Client 决策基线(D2 时未指定目录)
  -  — main IPC 拆分,D4 锁定 `src/main/features/<domain>/ipc.ts`
  -  — Phase 1 已完成 MCP 模块深化,本 ADR 完成目录统一
...
唯一 outlier:**MCP** — 13 个 `src/main/mcp-*.ts` 平铺在 `src/main/` 根,违反 locality 精神。

ADR-0032 (2026-07-21) 早于 (2026-08-01) 一周多。当时 `src/main/mcp-host.ts` 是单文件,与 mcp-config / mcp-manager 平铺在 main/ 根,符合当时惯例。ADR-0039 落地 features/ 约定后,MCP 没回头改 — (Phase 1) 深化 MCP 模块时也只关注 concern 拆分,未触及目录。
...
集成测试 import MCP 模块验证 channel 注册总数(从 30 → 36,per):
...
| 选项 | 描述 | 选 / 不选 |
| --- | --- | --- |
| 不动 | 维持 `src/main/mcp-*.ts` 平铺 | 不选 — locality 失守,MCP 是唯一 outlier |
...
- stdio-only / D3 naming rule / D4 tool call path
- 所有深化决策(模块拆分 + helpers + 本身)
- module-level singletons locality
- Test count:285(零变化)
...
```
chore(mcp): move MCP files to features/mcp/ — directory consistency
```
