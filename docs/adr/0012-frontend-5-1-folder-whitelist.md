...
- Date: 2026-06-15
- Scope: codeman-agent V1 前端 src/ 结构第二轮收口
...
动机：内置 AgentTool（file-ops、webfetch）与 `features/` 的业务域（chat、settings、billing）不同——它们是 LLM-facing 工具定义，不承载 UI 或业务状态。`features/<feature>/tools/` 曾在 被合并到 `lib/`，但 `tools/` 作为 renderer 顶层目录（`src/renderer/src/tools/`）是新增位置，不与现有 feature 目录冲突。
...
- 5+1 → 6+1 白名单新增 `tools/`，覆盖内置 AgentTool 定义
...
- 加 "Superseded in part by ADR-0010" 注

## References

- (Feature-Sliced 前端分层 + shadcn 风格 UI 原子) — V1.0 初版结构
- (Effect-TS 逻辑层) — UI 不导入 effect 规则不变
- (Tailwind v4 utility-only) — 不变
- (完整原生窗口 + TanStack Router) — 路由结构不变
- (开发者语言中文化策略 V1.6+) — 注释 / 治理文档 / 测试描述走中文
- 顶层 `AGENTS.md`
- `src/AGENTS.md`
