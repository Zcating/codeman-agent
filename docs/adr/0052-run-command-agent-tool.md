...
**Scope**:
- 新增: `src/renderer/src/tools/run-command/` (renderer tool — 同 webfetch 布局)
...
**Related**:
- (webfetch — 镜像布局与 IPC 模式)
- + 0013.1 (file-ops — 工具结构基线)
- (Effect Schema 全栈)
...
`@earendil-works/pi-agent-core` v0.80.3 不内置 shell/CLI 工具(经核查 `dist/index.d.ts` 与 `harness/utils/shell-output.ts` 均未导出 `AgentTool` 形态)。当前 Agent 工具集(经 收敛后):
...
snake_case 与 file-ops 族一致(`read_file` / `write_file` 等历史命名,见)。新工具沿用 snake_case 与 file-ops 视觉对齐。
...
6. **Task F** (commit `xxxxxxx`): + CONTEXT.md 更新
...
- (webfetch agent-tool — 镜像布局)
- (file-io-tools — 工具结构基线)
