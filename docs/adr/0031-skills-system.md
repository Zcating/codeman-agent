...
| 用户启用的 Skill 列表 | `Settings.enabledSkills: Set<string>` | 与现有 Settings JSON 同档 (per ADR-0015)；不引新存储 |
...
**采用 `~/.agents/skills/`**：与现有 agent 生态的 skill 目录**字面一致**,用户从 Claude Code / Cursor 迁移认知零成本;`~/.agents/` 根目录作为「agent 生态配置根」承载 skills + skills-lock.json + mcp_servers.json(per ADR-0032),统一品牌感。
...
- **Settings 单档保持**：enabledSkills 走现有 Settings JSON, 不引新表 (per ADR-0015)
...
- ADR-0019 D2 (per-run transient agent): systemPrompt 在 run() 时构造
- ADR-0025 (effect/Schema default): SkillManifest 走 Schema.Struct
- ADR-0015 (Settings 单 JSON 档): enabledSkills 走 settings-schema
- ADR-0010 (5+1 feature whitelist): skills 是第 4 个 feature (chat / settings / file-tools / skills)
