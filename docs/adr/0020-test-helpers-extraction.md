...
per ADR-0013)。这些 handler 必须在 `__mocks__/@tauri-apps/api/core.ts` 和
`src/test-setup.ts` **两处** 同步维护,否则 mock 路由漂移 → 测试偶发失败。

两处 source-of-truth 跟 ADR-0010 Q6 修复的 `mockState` 双源 bug 同性质。
...
└── src/
    ├── test-setup.ts                      (D3 删除)
    └── __mocks__/
        └── @tauri-apps/api/core.ts        (现有,per ADR-0010 唯一源;导出 mockState + invoke)
```
...
| 200+ 行 IPC `commandHandlers` 重复定义(per ADR-0010 单一源 bug) | **删除**——由 `__mocks__/@tauri-apps/api/core.ts` 单一源提供 |
...
### D4.`src/__mocks__/` 目录剩余内容(per ADR-0010)
...
- **`__mocks__/`** — IPC mock 自身,per ADR-0010 Q6 single-source,**不算 production code**
...
| `src/__mocks__/@tauri-apps/api/core.ts` | **不变**——继续是 mockState / invoke 唯一源(per ADR-0010) |
...
- ADR-0010 § "mockState 单一源" + § "5+1 白名单":`__mocks__/` 是 vitest 约定路径,
