...
- **Related**:
  - [ADR-0032](./0032-mcp-client-stdio.md) — MCP Client 决策基线(本 ADR 是其结构落地)
  - [ADR-0039](./0039-main-ipc-domain-split.md) — main IPC 拆分 precedent(per-domain register 函数 + state locality)
  - [ADR-0038](./0038-webfetch-agent-tool.md) — webfetch AgentTool 拆分 precedent
...
**拒绝**:
- A. functional factory `createStdioTransport(opts)`:项目惯例是 class-based stateful module(SettingsState ADR-0039 D5)。
- C. transport 直接 inline 在 `McpStdioServer`:违反 ADR-0039 拆分的 D3 locality 精神(transport 状态是独立 concern)。
...
**理由**:per ADR-0032 D3,`mcp_<server-name>_<tool-name>` 是项目锁定的命名规则(避免与 7 个内置 tool 撞名 + LLM 一眼识别来源)。命名 helper 集中这一规则,所有 caller 必然走同一路径 — 不会有「某个 caller 漏了 slug」之类 bug。
...
| 抽 transport + handshake + types | 不抽 `mcpAgentName` / 不抽 manager helpers | 不选 — 命名 helper 是 ADR-0032 D3 锁定规则,manager helpers 消除 restart/setEnabled 重复 |
...
- **Leverage**:未来 transport 扩展(V2 SSE/Streamable HTTP)— status 状态机可独立扩展(`McpServerStatus` 加 `kind: "sse_connecting"` 等);`mcpAgentName` 是 ADR-0032 D3 锁规则,所有 caller 必走
...
- ADR-0032 D2 stdio-only 约束 — 完全不变
- ADR-0032 D3 `mcp_<server>_<tool>` 命名规则 — **强化**(helper 集中)
- ADR-0032 D4 tool call 路径(main process spawn) — 完全不变
...
│   └── mcpAgentName(serverName, toolName): string  ← ADR-0032 D3 naming rule
...
docs: ADR-0041 — MCP module decomposition
```
...
- [ADR-0032](./0032-mcp-client-stdio.md) — MCP Client 决策基线(D2 stdio + D3 naming + D4 call path + D7 IPC)
- [ADR-0039](./0039-main-ipc-domain-split.md) — per-domain register 函数 + state locality precedent
- [ADR-0038](./0038-webfetch-agent-tool.md) — AgentTool main 拆分 precedent
