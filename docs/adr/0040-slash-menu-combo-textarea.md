...
**Related**: (Form 模式统一应用到 HomeAgentForm 与 ChatView — chat-view 已用 TanStack Form + `<form onSubmit>`，本 ADR 维持此架构), (codeman-* namespace), (Internal Components & Design Tokens)
...
└── commit 11（新增）: docs(adr): slash menu variants decision
```
