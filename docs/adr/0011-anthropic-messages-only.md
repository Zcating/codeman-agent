...
> **Status**: accepted · 2026-06-15 · 推翻 [ADR-0002](./0002-pi-mono-agent-runtime.md) 的"多 provider 假设"
...
- **(A) 多 provider**（沿用 ADR-0002 pi-ai 默认形态）—— V1 无需求，
...
- **(C) OpenRouter / LiteLLM 中转** —— 引入第三方代理层；V1 不接受，
  与 ADR-0010 的"无 Radix / 无 Kobalte"同理 —— 第三方抽象不在 V1 评估。
...
- **负面**：失去 V1 阶段对 OpenAI 生态（GPT / DeepSeek chat / Qwen / Ollama）
  ...
  推翻本决策。
- **不可逆**：本决策打破 ADR-0002 的多 provider 假设；恢复多 provider
  需要新开 ADR 论证 why-multi-provider-again。

## Supersedes

- ADR-0002 § 隐含的"pi-mono 多 provider 假设"（保留 ADR-0002 的
