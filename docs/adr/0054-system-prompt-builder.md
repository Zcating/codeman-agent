...
**Related**: (per-run transient agent — systemPrompt 在 run() 时构造), (skills system — skills 段注入), (multi-agents — 子代理独立 Agent), (pi route migration — provider adapter)
...
- D2: systemPrompt 在 run() 时构造
- D3: skills 段注入 system prompt
