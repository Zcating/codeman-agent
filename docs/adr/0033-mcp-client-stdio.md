...
**Related**: (Effect-TS logic layer), (5+1 feature whitelist — mcp 是第 5 个 feature), (per-run transient agent — MCP tools 在 run() 入口注入 tools[]), (effect/Schema), (Skills 是正交能力)
...
与 Skills 同根（per D-storage）。Schema：
...
- runtime `tools[]` 数组 = `createFileTools(...)` + `[loadSkillTool, ...mcpTools]` （per `_load_skill` + 本 ADR MCP tools）
...
**采用 main process spawn**：与现有 file-tools (per) 同构 — `tools[].execute` 走 IPC 到 main,main 执行 fs 操作
...
- (File IO tools) — 同构 IPC pattern (`tools[].execute` → `invoke(...)` → main process)
- (per-run transient agent) — tools[] 在 run() 时构造, MCP tools 每次 lazy fetch
- (Skills) — 职责正交;Skills 加知识,MCP 加能力
- (Settings 单档) — `~/.agents/mcp_servers.json` 与 Settings JSON 分立;遵循「agent 生态配置走 `~/.agents/`,app 配置走 Settings JSON」
