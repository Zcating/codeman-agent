...
**V2.8（chat-view V2.8，2026-07-29）**：拆出 `isAgentActive: boolean` 作为 per-message running 信号。`done` 仍清 `streamingMessageId`（per-turn 行为不变，符合 决策），但**不**碰 `isAgentActive`。`isAgentActive` 由 sendMessage 起始置 true，由新增的 `message_stop` RuntimeEvent 变体（runtime 在 `agent_end` 时 emit）置 false。chat-view 的 `isRunning()` 改读 `isAgentActive`，Send/Stop 按钮只在 message 真正结束时切一次，不在中间 turn 间抖动。
