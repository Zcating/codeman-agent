...
- **Supersedes**: 无 — 对 / D8 做**边界澄清**(不推翻,见 D1 / D2)
- **Related**:
  -  — Effect-TS 逻辑层;`platform-node` 禁令仅 renderer,本 ADR 首次在 main 进程引入 Effect 运行时原语(Scope / Fiber / Layer)
  -  — Queue-based Runtime(renderer 侧 Fiber/Queue 先例)
  -  — Electron 壳(main = Node 生态,Node 工具可直接使用)
  -  — MCP Client 决策基线
  -  — main IPC 域拆分(IpcDeps / state locality / sandboxHandler)
  -  — MCP 模块拆分(class-based manager 保留;StdioTransport deep module)
...
| 禁止 platform-node 全项目 | **错**。禁令仅 renderer(「webview 没有 Node」);main 是 Node 进程,不在禁令范围 | line 36 |
...
**与 的关系(澄清,非推翻)**:D8 锁的是「jsonrpc 不拆 seam / 不拆模块」(1 caller = 假 seam),本 PR 遵守 — jsonrpc 仍是 1 个模块、1 个 caller(`McpStdioServer` 经 `StdioTransport` 间接使用)。D8 的「不动」指**结构**,不指**实现范式**;本 PR 不动结构,只换内部实现。
...
**与 的关系(澄清,非推翻)**:D6 拒绝的是「McpManager 完全 functional 化」(mutable Map 状态改用闭包表达),本 PR 不做 — manager 结构原样。本 PR 做的是 transport **内部**实现替换 + 错误通道打通,两个不同轴。
...
- C. `McpManager` 全 Effect 化(`startAll` 返回 `Effect<void, McpBootError, Scope>`):违反 D6;manager 是协调器不是资源持有者,收益边际
...
- `db/mod.ts`:删 `_db` singleton / `initDatabase` / `getOrInitDatabase` / `closeDatabase` — 顺手修 Add-5.1 冗余 + 补「`closeDatabase` 从未被调用」缺陷
...
- 零散 `.catch(console.error)` 删除 — 失败经 `Cause` 统一进 logger(per)
...
- **jsonrpc 范式混存**(D1):class 外壳 + Promise public API + 内部 Effect runtime — 与 class 惯例一致,但读者需理解双层结构

### 不变

- renderer 禁令(D7 只是机器化,语义不变)
- `StdioTransport` deep module 外壳 / D6 `McpManager` class 结构 / D8 jsonrpc 不拆模块 / D9 公共 API
- IPC 域拆分 + IpcDeps + `sandboxHandler` 协议(throw-JSON 路径不变)
- IPC 错误契约(`{kind, message}` JSON)+ channel 名 + 参数类型 — **renderer 零改动**
- DB schema / `migrations/*.sql` / `_migrations` 表格式
- MCP stdio-only+ `mcp_<server>_<tool>` 命名
