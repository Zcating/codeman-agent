...
用户指定 `src/shared/stores/app.store.ts`（dot-separated），与 的 kebab-case 约定（`llm_providers.ts` → `llm-providers.ts` 修复）冲突。这是 后的**第二例命名例外**，需在 的白名单里追加说明。
...
`app.store.ts` 沿用 Pinia / Vuex 风格的 dot-separated 命名，**不**改为项目 kebab-case `app-store.ts`。在 的"5+1 子目录白名单"段追加"dot-separated 例外清单"：
...
V1 范围外，与 字面冲突（同 选项 A 否决 Node sidecar 的同样理由）。
...
- ProviderCard / WorkspaceCard 不再直接 `invoke`，符合 的 UI ↔ Effect 边界
...
- dot-separated 文件名 `app.store.ts` 是 后的第二例命名例外，需在 AGENTS.md 标记。
...
- 的"统一 Provider schema" —— providers[] 数组 + llm 必选 + billing 可选 不变；本 ADR 是其延伸（简化 key 子结构），不是逆转
