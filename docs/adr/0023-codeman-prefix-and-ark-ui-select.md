...
**Supersedes (in scope)**: ADR-0022 D5 (codeman-* prefix) + ADR-0022 D4 (hand-rolled Select — was proposed, not implemented); D6-H superseded by D8-W (this amendment)
...
V2.1 wave 1 + V2.1 polish 把 sidebar 渲染成 **两个平级 `SidebarGroup`**（Workspaces + Conversations），conv 列表由 `routes/index.tsx::itemsFromConversations()` 按 `appStore.selectedWorkspaceId()` 过滤。`CONTEXT.md` 定义 workspace 是 conversation 的「父」（per-Conv 绑定、一对多包含），但视觉上是平级而非嵌套——tree 语义和数据语义错位。下列决策在 ADR-0023 治理权范围内追加「cascade 显示」子规则。
...
- 状态由 Ark UI 内部 zag-js state machine 管理（uncontrolled via `defaultValue={[]}`），CodemanSidebar 不持有展开 signal，符合 ADR-0022 D3「codeman-* 组件严格 prop-driven」。
...
- 与 ADR-0016 D4（service-in-store）+ ADR-0022 D3（codeman-* 组件严格 prop-driven）一致——sidebar 不持有跨 feature 状态。
...
- cascade 复用 ADR-0023 D4-S 引入的 `@ark-ui/solid` 范式（已在 codeman-select.tsx 落地），不引入新依赖。
...
- File rename: `conversations.store.ts → chat.store.ts` (singular, aligns with public namespace `chatStore`; breaks ADR-0010 "store file = plural noun" convention — accepted trade-off for feature-name alignment).
