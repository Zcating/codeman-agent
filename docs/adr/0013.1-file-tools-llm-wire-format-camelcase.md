...
**Amends**: ADR-0013 ("B. File Tool 族" 表格字段名规则)
**Related**: ADR-0010 (5+1 文件夹白名单) · ADR-0025 (D8 type layer) · ADR-0024 (Electron shell) · `b6ce135 refactor(types): snake_case → camelCase in shared types and consumers` (已落地的 TS 内层迁移) · `7eb44a3 test(settings): snake_case → camelCase` (settings 域同方向迁移)

## Context

ADR-0013（V2 file IO 启动 ADR）在表格中把字段名定为 snake_case（`workspace_id` / `old_text` / `new_text` / `replace_all` / `content_pattern`），理由：当时 TS 内层 `Workspace` / `Conversation` / `Message` 都是 snake_case（ADR-0013 撰写时的代码状态），LLM-facing schema 复用同一套字段命名是阻力最小路径。
...
- ADR-0013 文档：保留 snake_case 表格作为 V2 启动时的设计快照，本 ADR-0013.1 显式 supersede 字段名规则（不重写 ADR-0013 全文）。
- Rust / Electron 后端（`electron/main/db/*` / `electron/main/ipc.ts`）：DB column snake_case 是 Rust 惯例，IPC channel 名 snake_case 是 Tauri 惯例，本 ADR 不动。
...
- `docs/adr/0013-file-io-tools.md`:
  ...
  - 新增 paragraph 段，引用 ADR-0013.1 supersede 字段名规则：
...
维持 ADR-0013 表格原状，file-tools schema field 保留 snake_case。
...
把 schema field 改回 snake_case（**反向**迁移）：让 ADR-0013 B 段字段名严格遵循 Rust convention，跟 DB column / Tauri command 名一致。

**否决理由**：
- 与 ADR-0025 D8 type layer 冲突 —— `Static<typeof toToolParameters(S)>` 会推导出 `{workspace_id?: string; path: string}`，TS 内部全是 `workspaceId`，dual-form 永远存在。
...
- **Review 心智模型切换**：开发者 review 一段 `pickArgs(args, "workspaceId")` 会本能怀疑"是否漏了 snake_case fallback" —— 需要 ADR-0013.1 来 wire-format 化。AGENTS.md / ADR 索引要在 PR 描述里显式 point to ADR-0013.1。
...
- **ADR-0013 修改一行**：line 41-46 表格前加 "Amended by ADR-0013.1" 段 —— historical snapshot 保留，字段名更新在 ADR-0013.1 段。

### 未变

- ADR-0013 的 5-工具架构 (read / write / edit / search / delete) 不变。
- ADR-0013 的 Hybrid 决策（webview agent + Tauri IPC fs syscall）不变。
- ADR-0013 的 Workspace-based sandbox（Rust 端 `canonicalize + starts_with`）不变。
- ADR-0013 的 Tauri command 签名（Rust 端 `fn read_file(workspace_id, path)` 等 snake_case）不变。
- ADR-0025 全栈 Schema 化 + SchemaToTypeBox<S> type layer 不变 —— schema field shape 自然现在 camelCase，仍然推导出 `{workspaceId?: string; path: string}`。
...
- 单 PR（branch `feature/file-tools-llm-camelcase-0013-1`），单 atomic commit。Commit message 模板：
  ```
  feat(file-tools): LLM wire-format rename snake_case → camelCase (ADR-0013.1)
  ...
  - ADR-0013 + ADR-0013.1 cross-link established
  ```
...
- Follow-up（不在本 PR）：
  - ADR-0026 候选：`src/features/chat/routes/index.test.tsx:236` 的 `workspace_id: "ws-1"` fixture + `root_path` / `updated_at` / `created_at` / `system_prompt` / `archived_at` 等 mock 数据是 chat 域 pre-existing drift，独立 ADR 处理。
  - ADR-0027 候选：Rust 后端 `electron/main/db/migrations/*` 列名如果未来跨进程 export 到 TS，需要 `serde(rename_all = "camelCase")` —— 当前不影响本 ADR。

## References

- ADR-0013 (V2 file IO) — 本 ADR amends B 段表格
- ADR-0025 (Effect Schema as Default Schema Library) — D8 type layer 推导出 `{workspaceId?: string; ...}` (改动后) 而非 `{workspace_id?: string; ...}` (改动前)
...
- CONTEXT.md "架构" section 文件工具 schema 子段 — 更新文档引用 ADR-0013.1
