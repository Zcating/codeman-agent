...
项目当前 sidebar 实现是 刚锁定（6 天前）的双层架构：
...
- B — 两层都推翻重来，包括 props 接口 — high-level contract（renderItem + 3 slots + ZERO business logic）已锁定，不推翻
...
### D5 — 保留 high-level contract

`renderItem` + 3 slots（header / footer / children）+ ZERO business logic 这四个 核心决策全部保留。实现层用 shadcn atoms 重写，但 API 契约不变。

**拒绝**：
- A — 趁重写机会改 props 接口 — 破坏 contract，consumer 全部要重写
...
1. **数据 props**:
   ...
   - `renderItem: (item: SidebarOption) => JSX.Element` (per)
   - `renderGroupHeader?: (group: SidebarGroupOption) => JSX.Element`
   - `currentValue?: string` + `isActive?: (value, currentValue) => boolean` (per)
...
- **D — 推翻 contract** — 拒绝：30 轮 grill 结论是保留 contract，只换实现
...
- **Layer 2 props 不变**（除 Q26 新增 `onSubItemSelect`）— contract 保留
...
| Q5 | 保留 contract     | renderItem + 3 slots + ZERO business logic                  |
...
| Q15 | currentValue + isActive 双 API | 保留                                           |
...
- (codeman-* namespace + Ark UI 选择)
- (sidebar 通用化 renderItem 全权控制)
- (Effect Schema 优先)
