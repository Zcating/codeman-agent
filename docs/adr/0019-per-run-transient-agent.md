...
**Supersedes**: ADR-0014 D1(`AgentRuntime` service 单例 + `Ref<Map<ConvId, Agent>>`)、D4(`Agent` 是 in-memory owner,DB 是 mirror)
...
`Agent` 累积 messages,DB 也存 messages;run 之间可能存在差异(比如 store 端有 tool_result 但 `Agent` 端没,因为 ADR-0014 D4 决定的"首次拉历史 + 后续累积"模式)。V1.5 早期已经因为 partial 不落库(per ADR-0014 D6:partial 保留在 `Agent` state)出过"切换对话 partial 消失" bug——`Agent` 与 DB 双 source-of-truth 增加心智负担。
...
根因:`messages.store` 全局 signal 设计 + chat-view 闭包绑 `convId` 写全局 signal。runtime 本身 per-conv(per ADR-0014 D1 + D5)是正确的,问题在 messages 状态层。

## Decision

3 个相互依赖的决策:

### D1.Runtime = 工厂函数(反转 ADR-0014 D1)
...
### D2.Agent = per-run transient(反转 ADR-0014 D4)
...
### D3.Store = single source of truth(新决策,supersede ADR-0016 的 store 拆分意图)
...
- ADR-0016 D4-D5-D6(chatAgentStore 抽象层)的"组件不直接 import runtime"约束仍然成立,只是中间层从 `agent.store` 移到 `conversations.store` 内 inline
...
- **Cancel 行为改进**:原 ADR-0014 D6 是"partial 保留在 `Agent` state",新设计 partial 在 store(subscription 实时写)。cancel 后用户切回 conv 还能看到 partial 进度(不是"丢失")
...
- 撤回 ADR-0019 + 重新激活 ADR-0014 D1/D4
- 改 `chat/AGENTS.md` 硬规则 + `CONTEXT.md` 词汇表
- 加 sidebar 状态点的反向移除(per ADR-0014 D5 UI 要求)
...
- ADR-0014 D1(原 `AgentRuntime` service 单例 + `Ref<Map>`):被 supersede
- ADR-0014 D4(原 `Agent` 是 in-memory owner):被 supersede
...
- ADR-0007 § "Future maintenance":单 main webview 约束下"后台"语义不变——per-conv `AgentRuntime` 实例在 webview 内存常驻
