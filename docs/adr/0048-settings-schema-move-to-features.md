...
- **Supersedes**: 无
- **Related**:
  -  — main IPC 拆分,D4 锁定 `src/main/features/<domain>/ipc.ts`
  -  — MCP 域同型 move(本 ADR 的直接先例)
  - [AGENTS.md](../../AGENTS.md) — 目录布局图(features/ 约定)

## Context

ADR-0042 (2026-08-02) 落地 MCP 13 文件 → `src/main/features/mcp/`。ADR-0042 D 段描述「唯一 outlier:**MCP**」是**不准确**的 — `settings-schema.ts` + 其 3 个支撑文件 (`schemas.ts` / `defaults.ts` / `sanitize.ts`) 同期也散在 `src/main/` 根,与 MCP 同属 locality 失守。

`features/settings/` 在 落地时已存在,持有 `state.ts` + `ipc.ts` 两个 module,正好是 settings 域的「runtime」关注点(`SettingsState` class + IPC handlers)。本次 move 落下的 4 文件是该域的「schema」关注点(`Settings` type + `DEFAULT_SETTINGS` + `sanitize()` + types + barrel),与 state/ipc 并列但不耦合:
...
保持文件名原样(per precedent + 用户确认):barrel 仍叫 `settings-schema.ts`(虽然 `settings-` 前缀在 features/settings/ 内部为冗余,但保持 API surface 不变 + 与 IPC type names / runtime identifier 视觉对齐)。同 MCP 决策:本次只 move 不 rename。
...
共 3 文件 4 行。**depth 缩短** 与 的 depth 增加方向相反(那里是 `src/main/` → `src/main/features/mcp/`,这里是 `src/main/` → `src/main/features/settings/` 但模块已在 features/settings/ 内部 → 实际是 sibling reference)。
...
| 选项 | 描述 | 选 / 不选 |
| --- | --- | --- |
| 不动 | 维持 `src/main/{settings-schema,schemas,defaults,sanitize}.ts` 平铺 | 不选 — locality 失守,5 个文件还是 outlier |
...
| move 整个 module | 4 文件 + 1 test 一起 move 到 `features/settings/`,无 rename | **选** — 最小 blast radius,机械操作,0 行为变化,与 同型 |
...
- **比 MCP 移动更干净**:depth 缩短(2 dots → 1 dot)而非增加,且外部 import 改动只有 4 行(MCP 11 行)
...
```
chore(settings): move settings-schema module to features/settings/ — round 2
```
...
-  — D4 main IPC 模块位置规则
-  — MCP 同型 move 先例(D1 文件名保留 + 用户 Option A 确认)
