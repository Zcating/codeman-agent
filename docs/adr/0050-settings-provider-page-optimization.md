...
- **Supersedes**: ADR-0047 中 "保留 Agent compat API、createProvider 仅作 Model 目录工厂" 方向不变;本 ADR 在此之上做 **UI/UX 层**优化,不逆转任何 runtime 决策
- **Related**:
  ...
  - [ADR-0043](./0043-settings-schema-move-to-features.md) — schema 双实现设计债
  - [ADR-0047](./0047-setting-provider-pi-route.md) — setting/provider 走 PI createProvider()
...
- `Default Model Invariant`(ADR-0016):`defaultModel ∈ models[].id ∪ {""}` 保留,在所有写路径强制。
