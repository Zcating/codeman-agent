...
- **Supersedes**: 无
- **Related**:
  - [ADR-0003](./0003-effect-ts-logic-layer.md) — Effect-TS 逻辑层基础(本 ADR 是其在 main 进程的现代化延伸)
  - [ADR-0017](./0017-queue-based-runtime.md) — Queue-based Runtime(renderer 侧 Fiber / Queue 先例;本 ADR 不涉及 Fiber/Queue,仅命名层)
  - [ADR-0024](./0024-electron-shell-migration.md) — Electron 壳(main = Node 生态,Effect 在 main 进程已被 ADR-0046 解禁)
  - [ADR-0039](./0039-main-ipc-domain-split.md) — IPC 域拆分(IpcDeps / state locality;本 ADR 不动 IPC handler 外壳,只动 handler 体内 effect 命名)
  - **母 ADR**: [ADR-0046](./0046-main-effect-ts-deepening.md) — main 进程 Effect-TS 深化(5 PR 落地 Fiber / Scope / Layer / DbLive / boot scoped);本 ADR 是其**第 6 条现代化线**,目标**仅**是 effectful function 命名 + locality 收口,不重复 ADR-0046 的范畴
  - [ADR-0041](./0041-mcp-module-decomposition.md) — MCP 模块拆分(McpManager class / StdioTransport deep module 不动)
...
- 1 个 mega-PR(~280L diff,review 不可行,违反 ADR-0046 D8「5 个独立 PR」精神)
...
```
1.  docs: add ADR-0059 effect-fn-modernization         ← 本 ADR
...
- 与 ADR-0058 PR-β 同模块,本 commit 是补完 wrap(ADR-0058 未把 wrap 进来)
...
- `index.ts`(1 处,boot sequence 内的 `Effect.gen(function*() { ... })` —— 已被 ADR-0046 D5 改造过 boot scoped,本 commit 仅 wrap 内层)
...
| 模块 | 理由 |
| --- | --- |
| `features/skills/skills-host.ts`(78L) | ADR-0046 D6 已锁,边界调用范式运行良好;本 ADR 也不迁 |
| `features/webfetch/handler.ts` / `ssrf.ts` | ADR-0046 D6 反候选,本 ADR 不触及 |
| `config-service.ts` | ADR-0046 D6 反候选,纯 env 解析 |
...
**实施**:本次改造**不引入** oxlint 规则(避免在 9 commits 里再插一个 PR),由 ADR-0059 + code review 守护。如未来 `src/main/**` 重新出现 `Effect.gen(function* () {`,在 code review 直接 block。
...
| 1 个 mega-PR | ~280L 单 PR | 不选 — review 不可行,违反 ADR-0046 D8 精神 |
...
**默认采用方案 A**(per ADR-0046 D8 精神)。

## References

- [ADR-0003](./0003-effect-ts-logic-layer.md) / [ADR-0017](./0017-queue-based-runtime.md) / [ADR-0024](./0024-electron-shell-migration.md) / [ADR-0039](./0039-main-ipc-domain-split.md) / [ADR-0041](./0041-mcp-module-decomposition.md) / [ADR-0046](./0046-main-effect-ts-deepening.md)
