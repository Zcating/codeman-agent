...
**Related**: (`codeman-sidebar` 首例 + internal 组件边界), D7-CS (Accordion uncontrolled 模式 + cascade 行为), (ProviderCard TanStack Form 模板 — sidebar 不是 form, 但 consumer 写法风格对齐)

## Context

`shared/components/internal/codeman-sidebar.tsx` (Wave V2.1 首例) 当前是 **chat 域专用** sidebar — 接受 `WorkspaceNode[]` + 8 个 chat 域概念 props (`onSelectItem` / `onDeleteItem` / `onCreateItem` / `onRenameWorkspace` / `onDeleteWorkspace` / `onEmptyWorkspaceClick` 等)，inline confirm 二次确认 UI 写死在 sidebar 内部 (line 83 / 234-275)，"workspace" / "conv" 概念泄漏到 shared/ 组件 (违反 "ZERO business logic, ZERO feature/store imports")。
...
### D5 — Accordion uncontrolled（按 D7-CS2）
...
**不暴露 `isExpanded` 给 consumer**：consumer 拿不到 group 当前展开态（与 D7-CS2 一致："组件内部 signal，不持久化、不耦合 appStore"）。如果 consumer 需要知道展开态，自己从 DOM 读 `data-state="open"` 或用 `createSignal` 自己管。
...
**好处**：sidebar 完全 prop-driven，符合 D3；chat 域代码跟 sidebar 解耦，未来 settings 域加 sidebar 项不会被 chat 概念污染。
...
**为什么迁出 store**：chat.store 不应该知道"sidebar 节点的 DOM 长啥样"——store 只存 domain data（workspaces / conversations），UI 形态在 component 层组装。这是 "store = domain state, no UI concerns" 的延伸。
...
- **E — 不抽 sidebar，直接在 chat-layout / settings-layout 里手写 `<aside>` + `<For>`**：**拒**：chat 域已有 298 行 sidebar 抽象，放弃抽象 = 反转，需要新 ADR 解释为什么不抽。
...
- **accordion 受控模式**：放弃（按 D7-CS2 uncontrolled）
...
3 个 PR，每个 1 个 atomic commit（参照 atomic rename 模式）：
...
| Q10 | Accordion 模式                    | uncontrolled（按 D7-CS2）                          |
...
| Q19 | `style` prop                      | 删（违反 无 inline style）                        |
