...
**Supersedes**: 部分 — 中关于 `AppError` 判别联合的实现细节（Error 模型 base 改变，但 "service-in-store" + formatAppError 公共 API 形态保留）

**Related**: (Effect-TS logic layer), (App Store + service-in-store + formatAppError), (unified logging), (Electron shell migration), `CONTEXT.md` "架构" section 新增 Schema 子段
...
PR 3 — file-tools typebox → Schema.Struct (5 schemas 改写)
  ├── ReadFileSchema = Schema.Struct({ workspaceId: Schema.optional(Schema.String), path: Schema.String })   (PR 5 后改 camelCase per)
  ├── WriteFileSchema = ...
  ├── EditFileSchema = ...
  ├── SearchFilesSchema = ...
  ├── DeleteFileSchema = ...
...
**拒绝理由（统一 shared/lib/schemas/）**：与项目 5+1 子目录白名单兼容，但跨域 ID 定义在 `shared/`，domain 边界模糊。Feature 自治符合 5 子目录白名单精神（domain knowledge belongs to domain）。
...
- **Effect 生态完整闭环**：`Effect` + `Layer` + `Stream` + `Schema.TaggedError` 全家桶，与 (Effect-TS logic layer) 一致。
...
- **PR 4 Branded 类型边界模糊**：跨域 ID（WorkspaceId）究竟定义在 file-tools / chat / shared 哪个？本次 PR 4 内评估；可能衍生。
...
每个 PR 单 commit（参照 atomic rename 模式）：
...
- Runtime path unchanged: JsonSchema.fromAST (per)
...
- 5 file-tool call sites now use AgentTool<typeof toToolParameters(...)>
  ...
  (Note: PR 5 落地时 schema field 是 snake_case `workspace_id`; 在 PR 5 之后将 LLM wire-format 改 camelCase `workspaceId`,执行参数类型同步更新。)
