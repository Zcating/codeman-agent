...
**Related**: ADR-0003 (Effect-TS 逻辑层), ADR-0009 (中文化策略), ADR-0011(default-log-level), ADR-0014 (Per-Conversation Agent), ADR-0016 (service-only-in-store / D4 硬规则), ADR-0017 (Queue-based Runtime), `src/shared/lib/logger.ts`, `src/shared/lib/tauri.ts`, `src/features/chat/lib/runtime.ts`, `src-tauri/src/commands/mod.rs`, `src-tauri/src/commands/filesystem.rs`
...
| ADR-0009 §4 | developer string（含 log / console / panic）→ 中文 | 是规则，不涉及具体日志库 / API 形状 |
...
3. **D4 硬规则（ADR-0016）** 规定组件层禁止 service 操作直接调用（`invoke` / `Effect.gen yield*` / `fetch`），但 `chat-view.tsx:13` 已直接 `import { Duration, Effect, Exit, Stream } from "effect"` 调 `Effect.runPromiseExit`，是 D4 灰色地带。

4. **redaction 规则**（`CONTEXT.md § Logging` 第 4 条"log 语句一律 redact api_key 字段"）是 ADR-0015 引入明文 API key 存盘时同步建立的安全规则；但实施成本（`LogCtx` 结构化参数 + 自动 redact 算法）与 simple API 形状（`logger.*(...args)` 与 `console.*(...args)` 1:1）冲突——本 ADR **降级**该规则为 developer 自觉（理由详见 D6）。

5. **tauri-plugin-log 2.x 默认单文件 append**，无 rotation。本期**不实现** rotation（详见 D7），开 ADR-0019 follow-up。
...
**不变量**：
- Rust 端 `Provider::fetch` 是 `secret.expose()` 唯一调用点（已 ADR-0014 强化）；其他位置对 `Secret<String>` 用 `&secret`，**不** `.expose()` 出函数外 → log 语句对 `Secret` 自动不会打印原文
...
- 它与 `console.log` 同类（output sink），按 ADR-0016 D4 精神"IPC + service 必须在 store" 不延伸
...
> **Secret** — Rust 端 `Secret<String>` newtype，`Debug` / `Display` 打印 `Secret(***)` / `***`。V1.7+ 后 Settings JSON 明文存 key，`Secret` 主要用于 pi-agent 运行时构造 header 时临时包裹。**调用方**：`logger.*` / `log::*!` 不得打印完整 secret 值（任一语言）；`Secret` 类型自动重载 `Debug` / `Display`，裸字符串变量需手动 redact 为 `***`。V1.10+ 起本规则从"强制 redact"降级为 developer 自觉——理由是 simple logger API 与自动 redaction 实现冲突，详见 ADR-0018 D6。
...
### D7. log rotation：本期不实现，开 ADR-0019 follow-up
...
| C | 本期不实现，开 ADR-0019 follow-up | **选** — scope 收住，rotation 独立 ADR 评估 |
...
- **tauri-plugin-log 单文件不轮转**：log 文件会持续增长。Mitigation：开 ADR-0019 follow-up；本期 V1 单机单用户使用频率下增长慢（每日 ~1MB）
...
- ADR-0003 (Effect-TS 逻辑层) "UI 不导入 effect" 约束 —— 仅 logger.ts 是 simple 输出 API（无 effect 导入），不破坏该约束
- ADR-0009 (中文化) developer string 中文规则 —— `log::*!` / `logger.*` 字符串继续中文（ADR-0009 §4 表不变）
- ADR-0011(default-log-level) 默认 Info 级 + `RUST_LOG` 开关 —— 不变
- ADR-0016 D4 硬规则 —— service 操作仍在 store；logger.* 不算 service 操作
- ADR-0017 Queue-based Runtime —— runtime.ts 仍返回 `Stream<R=never>`；本 ADR 仅在 `handleAgentEvent` 内部加 logger 调用，不改 Stream shape
...
- ADR-0019 follow-up：log rotation 策略（按大小 / 按日 / tracing 升级 三选一）

## References

- ADR-0003 (Effect-TS 逻辑层) — bridge pattern 保留；UI 不直接接 effect 约束不变
- ADR-0009 (中文化) — developer string 中文规则（log / console / panic / Err）继续生效
- ADR-0011(default-log-level) — `lib.rs::run` 默认 Info 级 + `RUST_LOG` 开关不变；本 ADR 在其基础上补充"哪些路径必打"
- ADR-0014 (Per-Conversation Agent) — per-conv Agent Map / lazy create / history feed 不变；RuntimeEvent 5 变体 shape 不变
- ADR-0016 (service-only-in-store) — D4 硬规则不变；logger.* 不算 service 操作（详见本 ADR D3）
- ADR-0017 (Queue-based Runtime) — runtime.ts Stream shape 不变；本 ADR 在 `handleAgentEvent` 内部加 logger
...
- ADR-0019 候选（follow-up）：log rotation 策略
