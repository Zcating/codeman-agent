# 0045 — skills-host.ts 拆分为 features/skills/ipc.ts + skills-host.ts(完成 全集落地)
...
- **Related**:
  -  — D4 main IPC 模块位置规则(`features/<domain>/ipc.ts`)
  -  — skills fs 侧同型 move(本 ADR 的直接上半截)
  -  /  — features/ locality 同型先例
...
> `src/main/skills-host.ts` 仍在 `src/main/` 根,违反 ("所有 6 个 register 函数位于 `src/main/features/<domain>/ipc.ts`")。

ADR-0039 D4 是 main-side features locality 的总规则,5 域(settings / conversations / workspaces / file-ops / system / webfetch) + MCP 早先已落地。skills 是最后一个 outlier。**本次 ADR 关闭这个最后的 gap,让 在 main-side 8 个域全集满足**(settings / conversations / workspaces / file-ops / system / webfetch / mcp / **skills**)。

### 当前状态(pre-ADR-0045)

```
src/main/
├── skills-host.ts                       ← 78L,4 关注点混在一起,违反
...
ADR-0044 落地后用户指出:「按 0039 模式,应该新增 features/skills」。本次 ADR 直接答这个问题:不只 move(ADR-0044 已做),还要把 register 函数挪到 字面要求的 `ipc.ts` 位置。
...
**理由**:
- 字面要求 register 函数位于 `<domain>/ipc.ts`,且 5 域均严格遵循
...
- 文件名 `skills-host.ts` 保留(per + precedent:rename 留作未来)

**拒绝**:
- A. 不拆单文件 `features/skills/skills-host.ts`:违背 字面,register 函数不在 `ipc.ts`
...
**命名依据**:
- 对齐 5 域命名: `registerSettingsIpc` / `registerConversationsIpc` / `registerFileOpsIpc` / `registerSystemIpc` / `registerWebfetchIpc`
...
- B. 保留 `registerSkillHandlers()`(零改名):失去 字面对齐

### D3 — 调用方式:直接调(对齐 MCP precedent),不走 `registerIpcHandlers` barrel

`src/main/index.ts` boot sequence(per):
...
- skills 选择对齐 MCP 而非 5 域的 barrel 编排
- 调用方式(直接调 / 走 barrel)和文件位置(`ipc.ts` / `mcp-ipc.ts`)是两个正交维度——skills 在「文件位置」上对齐,在「调用方式」上对齐 MCP

**拒绝**:
- A. 走 `registerIpcHandlers` barrel:要求 `registerSkillsIpc` 接 `getMainWindow` deps,实际不用(per Add-3 「getMainWindow 为死参数」),会引入死参数
...
| 选 | 描述 | 选 / 不选 |
| --- | --- | --- |
| 不动 | 维持 `src/main/skills-host.ts` | 不选 — locality 失守,skills 是最后一个 outlier |
| 只 move(ADR-0044 已做) | 只搬 fs 侧 lib/ | 不选 — IPC 文件仍在 root,未完成 D4 |
| move + 拆分(本 ADR) | D1-D7 全套: 拆 2 文件 + register 命名 + 直接调 + ipc.test.ts 补盲 + 单 commit | **选** — 完成 全集落地 |
| move + 拆分 + 走 barrel | 本 ADR + 把 `registerSkillsIpc` 加入 `registerIpcHandlers` 编排 | 不选 — 死参数 `getMainWindow`(per Add-3),打破 MCP precedent 一致性 |
...
- **0 行为变化**:公共 API 100% 不变,IPC handler 名 `skillsScan` / `skillsLoad` 不变(per channel 锁定),handler body 1:1 搬移
...
- `skillsScan` / `skillsLoad` channel 名(per 锁定)
...
- 测试计数 303 → 303(本 ADR 不新增 test case,只更新 `EXPECTED_CHANNELS` 列表 — 285 → 303 是 的 delta,本 ADR 不变)
...
→ 类型**定义在 renderer 侧,被 main + renderer 共同消费**。正确归属应是 `src/shared/lib/types.ts`(真正跨 main+renderer 共享)。本 ADR 沿用 不修策略(用户确认 type ownership 留作未来 ADR)。
...
```
refactor(skills): split skills-host.ts into ipc.ts + skills-host.ts — complete for skills
```
...
-  — IPC channel camelCase 锁定(本 ADR 保持 `skillsScan` / `skillsLoad`)
-  — D4 main IPC 模块位置规则(本 ADR 的总依据)
-  — MCP 域同型 move(直接先例)
-  — settings-schema 域同型 move
-  — skills fs 侧同型 move(直接上半截)
- [AGENTS.md](../../AGENTS.md) — features/ 目录约定 + atomic commit / --no-ff 约束