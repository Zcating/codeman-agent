...
**Related**: (锁定 pi-mono runtime — automation 的 LLM action 走 pi-mono `Agent`), (app-store/key 简化 — Provider 凭据复用), (per-run transient agent — 每次 LLM action 新建独立 `Agent`), (effect/Schema — 配置与 execution 表 schema 来源), (form mode — Settings form 字段走 TanStack Form), (Renderer Plugin Registry — 新 plugin 沿用 `PluginDescriptor`), (主内容滚动区 — plugin tab 是 ScrollArea 页面), (skills IPC split — automation IPC 模式借鉴), (run_command — script action 沙箱与超时复用), (multi-agents plugin — 同为 renderer plugin 第四个 builtin)
...
- **Renderer Plugin Registry 已存在**(`src/renderer/src/plugins/`),三个 builtin plugin:`skills`、`mcp`、`multi-agents`。PluginDescriptor 形状: `{ id, initialize: Effect, route, sidebar }`(per)。
- **持久化模式二选一**: Skills/MCP 走 `~/.agents/*.json`(per),Multi-Agents / Conversations / Workspaces 走 SQLite。配置类型选 JSON,运行日志选 SQLite。
- **pi-ai / pi-agent 仅在 renderer 端可用** — `createAgentRuntime`、`createSubAgent`、`anthropicStream` 全部引用位于 `src/renderer/src/features/chat/lib/`。**Main 端无法直接调 LLM,必须 IPC 跳 renderer**。
...
- **run_command 沙箱模式** — `src/main/features/run-command/` 实现 workspace 路径校验(`fs.realpath()` 后检查路径是否在 workspace root 内),可直接复用为 script action 的沙箱(per)。
...
走 `~/.agents/` 路径,与 skills / mcp 同根,不引 Settings JSON(避免污染主档)。
...
// LLM action: IPC 跳 renderer spawn sub-agent(per sub-agent-factory 模式)
...
CONTEXT.md 新增 `### Automations (V3.2,)` 段,定义:
...
- `stores/automations.store.ts` — Solid store + IPC bridge(per)
- `stores/executions.store.ts` — execution history Solid store
- `components/settings-tab.tsx` — plugin tab 主页(per ScrollArea 页面)
