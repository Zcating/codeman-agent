...
V1.6+ per-conversation Agent runtime使用 `Stream.unwrap + Effect.gen + Stream.async` 三层 pattern：
...
const run = (
  ...
  return Stream.scoped(
    Effect.gen(function* () {
      ...
      // 获取 / 创建 agent（保留 D1+D4 的 lazy-create + history feed 逻辑，不变）
...
| B   | 保留 `SettingsService \| MessageService` lie | 不选 — doc lied；继续标 "known issue" 是技术债 |
...
- **chatAgentStore API surface 不变**：3 个方法签名同
...
- **V1.9+ sprint**：supersede 部分 的 runtime pattern
...
- **ADR-0018 候选**：`getSettingsBridge` 跟 的剩余迁移（theme.ts 是 #7 标记的最后一个 store），加上 `Effect.runPromise` 在 settings 加载阶段的 service-not-found 根因分析。
- 9 个 failing e2e spec 的真实 fix 不在本 ADR 范围，依赖 / 后续 issue 解决。
