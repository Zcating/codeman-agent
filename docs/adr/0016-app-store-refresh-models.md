...
**Supersedes**: 无 (与 ADR-0015 平行扩展)
**Related**: ADR-0003 (Effect-TS 逻辑层), ADR-0010 (5+1 白名单), ADR-0012 (Unified Provider schema), ADR-0014 (Per-Conversation Agent), ADR-0015 (appStore), CONTEXT.md Provider.llm / Provider.billing / ModelMeta / ProviderService / App Store
...
`ProviderCard.handleRefreshModels` 把"拉取 + 写 state + 调度持久化"塞进 component,违背 ADR-0015 的"UI → Effect → store"分层。同样的反模式在 `workspace-card.tsx:54-69` (`handleBrowse`)、`settings.tsx:72-79` (`clearHistory`)、`chat-view.tsx:85-93` (cancel) + `121-195` (startRun) 也存在。
...
### 触发 4: service 操作散在组件层,违反 ADR-0003 / ADR-0015 分层
...
**`settingsSaver.scheduleSave()` 仍由组件调**(保持 ADR-0015 的 "shared → feature 单向依赖" 硬约束 — store 不引 `features/settings/lib/settings-saver`)。
...
- ADR-0015 的"shared → feature 单向依赖"硬约束 — `settingsSaver.scheduleSave()` 仍由组件调,store 不引 feature lib
- ADR-0010 5+1 白名单 — `appStore` 仍在 `src/shared/stores/`,`chatAgentStore` 在 `src/features/chat/stores/`,新 helper `formatAppError` 在 `src/shared/lib/`
...
- ADR-0003 (Effect-TS 逻辑层) — D4 强化其"UI 不直接接 Effect 错误"约束
- ADR-0010 (5+1 白名单) — store 位置约束
- ADR-0012 (Unified Provider schema) — `Provider.llm` / `Provider.billing` shape
- ADR-0014 (Per-Conversation Agent) — D6 沿用其 `AgentRuntime.run` Stream API
- ADR-0015 (Settings 全局 app-store) — 本 ADR 是其 API 扩 + 加 D4 硬规则
