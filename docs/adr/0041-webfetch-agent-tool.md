...
**Related**:
- (file-tools design reference)
- (schema field camelCase)
- (Effect Schema full-stack)
- (IPC channel camelCase)
- (MCP capability extension reference)
- (5+1 whitelist, amended by this ADR)
...
`src/renderer/src/features/file-tools/` 自 起放在 `features/` 下，但 file-tools 是 LLM-facing AgentTool 定义——不承载 UI、不涉及业务域、不与 chat/settings/billing 共享 feature 模式。将其迁至 `tools/file-ops/` 并扁平化（删除 `lib/` 嵌套）与 webfetch 工具同目录平级，形成 `tools/<name>/` 一致布局。同时清理 `features/` 下非 feature 语义的遗留代码。
...
与 一致: LLM 不会 abuse 自己，permission 弹窗无意义。无频率限制、无用户确认。SSRF 是唯一安全边界。
...
4. **Task D** (commits `bdbf056` + `e63f32a`): file-tools → file-ops migration + barrel + runtime.ts 注入 webfetchTool + 5+1 → 6+1 docs + 草拟
5. **Fix pass** (commit `59b3ef0`): 12 reviewer findings (AppError 包装、SSRF redirect、XSS 标签、lint、ADR 数字修正)

## References

- (5+1 whitelist, amended by this ADR)
- (file-io-tools design — file-ops 前身)
- (camelCase wire format)
- (Effect Schema full-stack)
- (IPC channel camelCase)
- (MCP client — parallel agent extension pattern)
