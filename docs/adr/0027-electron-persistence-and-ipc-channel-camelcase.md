...
**Amends**: (D10 amend 由 fd36f6207 在 master 已落地 schema rename + migration,本 ADR 跟进 transport layer 完成)

**Related**:
- / (file-tools wire-format rename) — 直接 precedent,本 ADR 是其向 Electron transport layer 的扩展
- (Provider.apiKey 单字段) — 同方向
- + D10 (V3.1 兼容 amend) — schema 字段已 camelCase,本 ADR 跟进 IPC channel + dual-form
- (effect/Schema 全栈) — D8 type layer 与 settings-schema 对齐
- `fd36f6207 fix(e2e): wire format snake→camel  + qa fixtures + UNIQUE constraint` — 本 ADR 的 schema 部分已由这个 commit 落地;本 ADR 锁定 transport 剩余
...
- 历史 ADR prose 中提到的 snake_case 字段名(`api_key` 等)只在已落地的 是 V3.1+ 状态描述;其他 ADR 是 V2 时代决策记录,保留原文是 record-of-why 的完整性。
...
- File Tool 的 LLM-facing tool name (`read_file` / `write_file` / `edit_file` / `search_files` / `delete_file`) 仍是 snake_case,Anthropic tool dispatch protocol 锁定 (per +)
...
- Line 36 (File Tool term):`replace_all` → `replaceAll` (per)
...
- Line 143 (Domain shape edit_file):`replace_all` → `replaceAll` (per 配套更新)
- Line 165 (Domain shape Provider.llm):`{ defaultModel, baseUrl, apiType, models[], modelsEndpoint }`
- Line 171 (Settings schema description):移除 `src-tauri/src/settings.rs` 历史 reference + 新增 cross-link + `autoArchiveAfterDays` / `maxHistory` sanitize prose
...
- 跟 不对称(file-tools 内层 strict camelCase,IPC arg layer 还留 dual-form 是内部矛盾)
...
保留 snake on-disk + runtime transcode layer (与 类似):
...
- **blame diff 散落在 fd36f6207 + ADR-0026**:schema rename 在 fd36f6207,transport rename 在。Reader 看 git log 时需要穿越 2 个 commit,migration rename 的 rationale 需要 + 两个引用
...
- **V2 caller silent rejection**:无 graceful error path,V3.0 + D10 + 之后所有 V2 IPC contract 已终结
...
- File Tool wire-format schema field (`workspaceId` / `oldText` / `newText` / `replaceAll` / `contentPattern`) —— per 已落地
...
```
feat(electron): IPC channel rename snake_case → camelCase + drop dual-form
...
- File Tool term: replace_all → replaceAll (per 配套)
...
Reference:
- supersede fd36f6207 剩余 transport tail
...
- (V2 file IO) — B 段 table 字段名被 supersede
- (file-tools wire-format rename) — 直接 precedent,本 ADR 是其 Electron transport layer 对应
- (Provider.apiKey 单字段) — 同方向(TS 内层 + provider record)
- + D10 (fd36f6207 落地) — schema rename 在 schema 层完成,本 ADR 跟进 transport 层
- (effect/Schema 全栈) — D8 type layer 与 settings-schema 对齐
- `fd36f6207 fix(e2e): wire format snake→camel ` — schema 字段已落地
- `b6ce135 refactor(types): snake_case → camelCase in shared types and consumers` — TS 内层现状
