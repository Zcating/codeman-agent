...
**Related**: ADR-0002 (锁定 pi-mono runtime — sub-agent 仍走 pi-mono `Agent` class), ADR-0014 D5 (per-conv 1 active 流 — sub-agent 在 tool execution 内运行,**不冲突**), ADR-0017 (Queue-based runtime — sub-agent 工具调用是 tool execution,不触发 Conv active 流), ADR-0019 (per-run transient agent — 每个 sub-agent 一次 `run()` 新建 `Agent`), ADR-0028 (Bubble boundary — sub-agent 输出作为 tool result 进主 bubble,**不创建新 bubble**), ADR-0031/0032 (Skill/MCP 正交 — multi-agents 是新维度,不取代), ADR-0035 (Renderer Plugin Registry — 新 plugin 沿用 `PluginDescriptor` 注册), ADR-0048 (run_command AgentTool — multi-agents sub-agent 可调 run_command,作为其 allowedTools 之一)
...
- **codeman-agent 现状**: 单 `Agent` per conv,per-run transient (ADR-0019)。`Agent` 类支持 `toolExecution: "parallel"`(默认),可在同一 turn 内并发执行多个 tool call — **天然支持并发 sub-agent**。
- **现有约束**: 每 conv 至多 1 active 流 (ADR-0014 D5);bubble boundary = agent-turn boundary (ADR-0028);Skill/MCP 正交 (ADR-0031/0032);单 main webview (ADR-0007)。
...
- `allowedTools` 必须是当前可用工具名空间内的真名(主 agent 可见的 tool 名)。若工具名后续被禁用,sub-agent 启动时**静默跳过该 tool**(per ADR-0046 fallback 风格)。
...
走 ADR-0015 既有 Settings JSON 档,不引新 SQLite 表(与 Skills `enabledSkills` / MCP server config 同档原则一致)。
...
**关键**: 当用户**未配置任何 enabled sub-agent** 时,**不注入** `delegate_task` 工具(避免给 LLM 投毒)。per ADR-0019 runtime 是 per-run transient,用户修改 sub-agent 配置后**下一次主 agent 消息**即生效。
...
- 「+ Add sub-agent」按钮 → 弹 Codeman Dialog 编辑表单(基于 form 模式 per ADR-0029)
...
`src/renderer/src/plugins/index.ts` 改 1 行:加 `pluginRegistry.registerPlugin(multiAgentsDescriptor)`。Per ADR-0035 单插件失败不阻塞首屏。
...
- **Per-run transient 兼容**: Sub-agent 每次 `run()` 新建 Agent,符合 ADR-0019。无状态泄露。
- **Bubble boundary 不破坏**: Sub-agent 输出作为 tool result 进主 bubble,UI 无新 bubble 概念,符合 ADR-0028。
...
- **Plugin 启动开销**: 启动时 plugin registry 注册 multi-agents(几乎 0 开销,沿用 ADR-0035)。
...
- 现有 PluginRegistry 模式: ADR-0035 + `src/renderer/src/plugins/lib/plugin-registry.ts`
- runtime.ts 集成点: ADR-0017 + ADR-0019 + ADR-0028
- Skill/MCP 正交职责: ADR-0031 + ADR-0032
