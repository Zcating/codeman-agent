...
**Scope**: 9 文件 (5 重写 + 1 新增 + 2 同步 + 1 改构) + ADR-0034
...
- `accordion.tsx` / `sidebar.tsx` / `tooltip.tsx` — ADR-0033 已基于 `.repos/shadcn` 重写完
- `dialog.tsx` — ADR-0023 D8-W6 已基于 shadcn 风格完成 (不含 `data-slot`, 与 2025+ 风格有差异; 但本轮不改)
...
- ADR-0034 记录 9 文件决策, 后续 wave 缺失组件参考本 ADR 范式
...
- ADR-0033 — sidebar / accordion / tooltip shadcn 重写范式
- ADR-0023 D4-S — Select via `@ark-ui/solid` 范式
- ADR-0023 D8-W6 — Dialog pattern 参照
