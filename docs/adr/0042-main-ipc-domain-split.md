...
- **Related**:
  -  — renderer 侧 5+1 → 6+1 域拆分精神
  -  — IPC channel camelCase rename
  -  — `chat.store.ts` Store = single source of truth
...
- IPC channel name 字符串（per 已锁定 camelCase）
- IPC handler 的 arg types（per 已 strict camelCase）
- Anthropic transport 链路（renderer 侧，不在 main）
- DB schema / migrations（per D10）
...
```
refactor(main): split ipc.ts by domain + per-domain IpcDeps + state locality
...
- registerWebfetchIpc        (1 handler: webfetch:fetch — moved to features/webfetch/ipc.ts per)
...
Reference: supersedes nothing; aligns with renderer-side split spirit.
```
...
-  — renderer 5+1 → 6+1 域拆分精神
-  — IPC channel camelCase rename（已锁定 channel name）
-  — chat.store.ts = single source of truth
-  — V3 Electron shell（main / preload / renderer 三段）
-  — webfetch AgentTool（webfetch 已在 features/webfetch/）
...
**豁免理由**: 明确要求 `getOrInitDatabase` 作为独立入口,删除会偏离 spec。保留并接受这一层包装作为 D2 deps locality 的边界情况,后续如有清理 task 可二选一收敛。
