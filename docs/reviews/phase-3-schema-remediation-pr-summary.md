...
**Date:** 2026-07-11
**Related:** ADR-0025 · ADR-0025.1 — ADR-0025 Phase-3 Corrections
...
Closes the 12-review-finding remediation of Phase 3 (ADR-0025 PR 1–4) on `master@559d0c9`. Adds the Phase-3 amendment ADR-0025.1, fixes 9 of the 12 findings, and explicitly defers 2 (Task 2 typebox transitive-dep ambiguity; Electron IPC payload-shape migration).
...
a6b73b4 docs(tool-schema): align doc-comment with JsonSchema.fromAST (ADR-0025.1 D-C)
...
2322397 docs(adr-0025.1): amend D4 architecture, SandboxViolation.message, D8 helper
```
...
|---|---------|-----------|--------|
| 1 | Spec Deviation #1 (D4 公共基类方案) | Task 1 (ADR-0025.1 D-A: 8 独立 leaf classes + union) | ✅ closed |
| 2 | Spec Deviation #2 (D4 SandboxViolation.message) | Task 1 (ADR-0025.1 D-B: optional 保留 + deprecation 注释) | ✅ closed |
| 3 | Spec Deviation #3 (D8 helper: `Schema.toJsonSchema` vs `JsonSchema.fromAST`) | Task 1 (ADR-0025.1 D-C) + Task 5 (doc-comment alignment) + Task 7 in `tool-schema.ts` | ✅ closed |
...
- **New `Schema.TaggedError` instance shape is the only AppError production form** (per ADR-0025 D4 + ADR-0025.1 D-A). 8 independent leaf classes — no shared base class.
...
- **`toToolParameters`** (`src/shared/lib/tool-schema.ts`) uses `JsonSchema.fromAST(schema.ast, { definitions: {} })` per ADR-0025.1 D-C.
- **`workspaceIdField`** (new constant in `src/features/file-tools/lib/file-tools.ts`) replaces 5x duplicated `Schema.optional(Schema.String)` per Task 4 / ADR-0025.1-implied dedup.
...
**Resolution path:** either accept the devDep (amend ADR-0025 D2 wording via ADR-0025.1.1); migrate source imports to `typebox` (unscoped); or wait for upstream `pi-mono` packages to migrate.
...
**Resolution path:** migrate `electron/main/ipc.ts:413` to emit `{_tag: err._tag, message: err.message, ...}`. Likely small, can be tracked as ADR-0025.1.2 or a separate IPC cleanup ticket.
...
- Open **ADR-0025.1.1** (or ADR-0026) to amend D2 wording per Task 2 deferral.
- Open **ADR-0025.1.2** (or follow-up IPC ticket) for Electron IPC `_tag` migration.
