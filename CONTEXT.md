...
- **Scroll Region (滚动区)** — 主栏（`ResizablePanel#main`）内**恰好一个**垂直滚动容器承载路由内容滚动的布局契约，per [ADR-0039](./adr/0039-main-content-sole-scroll-region.md)。载体是 shadcn `ScrollArea`（`ui/scrollarea.tsx`，`.repos/shadcn/scroll-area.tsx` 的 Solid 移植，基于 `@ark-ui/solid/scroll-area`），**契约标记 `data-scroll-region` 与 `data-testid` 落在 Viewport**（真正的滚动元素，zag 注入 `overflow:auto`）而非 Root。两个消费方：内容 wrapper（`data-testid="main-content-scroll"`，承载**非 chat 路由**的滚动）与 ChatView 消息区（承载 chat 内层滚动），均 `class="flex-1 min-h-0"`。**不变量**：主栏内**活动滚动区**（`scrollHeight > clientHeight`）恰好一个——非 chat 页是 wrapper viewport，chat 页是消息区 viewport（wrapper 因 `conversation-route` `h-full overflow-hidden` 恰好贴合、无溢出）。**历史违规**：V2.9（`SidebarInset` 加 `overflow-y-auto` → 双滚动条 + 工具栏滚走）、V2.10（移除后 wrapper 未补滚动通道 → 非 chat 页无法滚动）。**强制机制**：单测（`scroll-region.test.ts` 断言模块 + `scrollarea.test.tsx` 契约测试 + `codeman-sidebar.test.tsx`）+ e2e（`03-layout-scroll.spec.ts`：超高页 wheel 生效 + 工具栏钉住 + 恰好一个活动滚动区）。_避免_：在任意路由再自建 `overflow-y-auto` 容器（违反"恰好一个"不变量，V2.9 复发）；移除 wrapper 的滚动能力而不补替代通道（V2.10 复发）；把 `data-scroll-region`/`data-testid` 放到 ScrollArea 的 Root 上（Root 不是滚动元素，`scrollHeight` 断言会失效）；叫 scroll area / scroll container（与 shadcn `ScrollArea` 组件名混淆——组件用 `ScrollArea`，布局契约用「Scroll Region」）。
