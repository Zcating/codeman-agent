...
- **Automation (自动化)** — 端用户配置的定时任务单元,持久化于 `~/.agents/automations.json`。由 renderer Plugin Registry 第 4 个 builtin plugin (`automations`) + Electron Main `AutomationScheduler` 协同承载:renderer 负责 UI + IPC + LLM action 执行,Main 负责 setInterval 调度 + 持久化 + script action 执行 + IPC 注册。per [ADR-0053](./adr/0053-scheduled-automation-plugin.md)。_避免_:把 automation 当成「AgentTool」(它跨进程,不是单 tool);把它归入 Settings 顶层(它是 plugin 家族成员,与 skills/mcp/multi-agents 平级)。
- **Automation Rule (自动化规则)** — 单条 automation 配置 shape:`{ id, name, enabled, schedule, action, createdAt, updatedAt }`。`schedule` 是 `interval` / `daily` / `weekly` 三选一(无 cron 表达式,human-readable only);`action` 是 `llm` / `script` 二选一(用户在 rule 上二选一,不可同条 rule 混用)。per ADR-0053 D1。_避免_:每条 rule 必填 timeoutMs(5min 默认 + 用户可改,简化 UI);schedule 支持 cron 表达式(增加门槛与依赖,不引 cron-parser)。
- **Automation Schedule (调度计划)** — human-readable 表达:interval (every N min/h) / daily (HH:MM) / weekly (DDD HH:MM)。6 预设 (every 5m / 15m / 30m / 1h / 6h / 1d) + 自定义 radio。nextDelay 计算纯函数,不引 npm 依赖。per ADR-0053 D5。_避免_:接 cron 标准表达式(用户门槛高,80% 用例 human-readable 已覆盖)。
- **Automation Action (自动化动作)** — 触发时执行的具体形态。
  - `llm` — `{ systemPrompt, userPrompt, providerId, modelId, timeoutMs }`。**执行路径**:Main IPC 跳 renderer(`automations:execute-llm`),renderer 端按 ADR-0049 sub-agent-factory 模式创建独立 `Agent`,跑 `agent.prompt(userPrompt)`,返回 final assistant text。**为何不在 Main 端跑**:pi-ai / pi-agent 仅在 renderer 可用,Main 端无 LLM client。per ADR-0053 D6。
  - `script` — `{ language: "shell" | "javascript", source, workspaceId, timeoutMs }`。**执行路径**:Main 端 `child_process.spawn`,**复用 run_command 的 workspace sandbox**(per ADR-0048):`fs.realpath()` 后检查路径是否在 workspace root 内,越界返回 `SandboxViolation`。5min 默认 timeout。per ADR-0053 D6。
...
- **Automation Execution (自动化执行)** — 单次触发的实例,持久化于 SQLite `automation_executions` 表(per ADR-0053 D3,Migration 0007)。shape:`{ id, rule_id(软 FK), status, trigger_kind, started_at, completed_at, duration_ms, final_text, exit_code, stderr, error, metadata_json }`。与业界主流(GitHub Actions `workflow_run` / n8n `ExecutionEntity` / Zapier run summary)对齐,只存 metadata + final output,**不存中间 messages**。per ADR-0053 D8。_避免_:进 conversations 表(语义污染,与业界不同);存 messages[] 进 DB(实现复杂 + 体积大)。
- **Trigger Kind (触发类型)** — execution 的来源枚举:`scheduled`(scheduler setInterval 到点)/ `manual`(用户点 ▶ Run now 按钮)/ `missed-replay`(启动时检测到错过,用户主动补跑)。每种 trigger_kind 状态机相同,差别在于 UI 标记与"是否计入 missed 列表"。per ADR-0053 D11/D18。
- **Missed Run (错失执行)** — 应用未启动期间本应触发但未执行的 scheduled run。**检测阈值 = 1× period 严格**(若 now > lastCompleted + period,记 missed)。启动时枚举 missed rules,UI 列出待补跑,**用户主动 Run now 补跑,不自动补**(避免 token 账单惊吓)。per ADR-0053 D11/D12。_避免_:启动时自动跑所有 missed(token 风险 + 重复跑);丢(失去审计)。
- **AutomationScheduler (Main 端调度器)** — `src/main/features/automations/scheduler.ts` 单例。`app.whenReady()` 钩子里 `scheduler.start()`(读 `~/.agents/automations.json`,为每条 enabled rule 注册 setTimeout);`before-quit` 钩子 `scheduler.stop()`。**per-rule FIFO queue**:同一 rule 在 status=running 时新触发进 queue,等前一个跑完(防止 race 与重复);不同 rule 并行。per ADR-0053 D5。_避免_:lock 全局 semaphore(限制并发);skip overlap 静默丢(丢语义)。
