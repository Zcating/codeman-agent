...
- **Scroll Region (滚动区)** — 主栏（`ResizablePanel#main`）内**恰好一个**垂直滚动容器承载路由内容滚动的布局契约，per [ADR-0039](./adr/0039-main-content-sole-scroll-region.md)。载体是 `ui/scroll-region.tsx` 的 `ScrollRegion` 原语（class 三件套 `flex-1 min-h-0 overflow-y-auto` + `data-scroll-region` 身份标记），两个消费方：内容 wrapper（`data-testid="main-content-scroll"`，承载**非 chat 路由**的滚动）与 ChatView 消息区（承载 chat 内层滚动）。**不变量**：主栏内**活动滚动区**（`scrollHeight > clientHeight`）恰好一个——非 chat 页是 wrapper，chat 页是消息区（wrapper 因 `conversation-route` `h-full overflow-hidden` 恰好贴合、无溢出）。**历史违规**：V2.9（`SidebarInset` 加 `overflow-y-auto` → 双滚动条 + 工具栏滚走）、V2.10（移除后 wrapper 未补滚动通道 → 非 chat 页无法滚动）。**强制机制**：单测（`scroll-region.test.ts` 断言模块 + `codeman-sidebar.test.tsx`）+ e2e（`03-layout-scroll.spec.ts`：超高页 wheel 生效 + 工具栏钉住 + 恰好一个活动滚动区）。_避免_：在任意路由再自建 `overflow-y-auto` 容器（违反"恰好一个"不变量，V2.9 复发）；移除 wrapper 的滚动能力而不补替代通道（V2.10 复发）；叫 scroll area / scroll container（与 shadcn `ScrollArea` 语义混淆）。
