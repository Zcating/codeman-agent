...
| 新增 sidebar 组件                 | 改 `shared/components/ui/sidebar.tsx`（primitive） + `shared/components/internal/agent-sidebar.tsx`（业务组合），遵循 [ADR-0022](../docs/adr/0022-internal-components-and-design-tokens.md) |
...
| 新增跨域业务组件                  | `shared/components/internal/<Name>.tsx`（**ADR-0022** 首例 `agent-sidebar`，新组件须严格 prop-driven）                              |
