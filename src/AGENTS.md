...
│   ├── balance-view.tsx   # DeepSeek 显示(¥ 87.42 + 自动续费)
│   ├── plan-quota-view.tsx # MiniMax 显示(1.2M / 5.0M + 进度条)
│   ├── stale-badge.tsx    # "stale" 角标,1 秒一跳
│   ├── switcher.tsx       # 两个圆点的厂商切换器
│   ├── settings-app.tsx   # 设置窗根组件
...
- **文件名用 kebab-case,导出的组件名保持 PascalCase。** 例如 `balance-view.tsx` 导出 `BalanceView`。多词文件名一律连字符;单词文件保持小写(`app.tsx`, 不写 `App.tsx`)。新增文件前先核对这条规则。
...
| 新增设置 tab | `components/settings/<name>.tsx`,在 `settings-app.tsx` 的 `SECTIONS` 注册 |
