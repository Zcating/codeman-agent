...
describe("errors — Schema.TaggedError variants (ADR-0025 PR1)", () => {
  it("each variant carries its OWN instance _tag (not a shared base)", () => {
    ...
    expect(new ToolCall({ tool_call_id: "t1", message: "a" })._tag).toBe("ToolCall");
    expect(new SandboxViolation({ path: "/p", workspace_label: "w" })._tag).toBe("SandboxViolation");
  ...
  it("optional fields accept present and absent values", () => {
    ...
    expect(new SandboxViolation({ path: "/p", workspace_label: "w" }).message).toBe("");
    expect(new SandboxViolation({ message: "m", path: "/p", workspace_label: "w" }).message).toBe("m");
  });

  it("required per-variant fields are accessible", () => {
    const tc = new ToolCall({ tool_call_id: "call_9", message: "boom" });
    expect(tc.tool_call_id).toBe("call_9");
    const sv = new SandboxViolation({ path: "/etc/passwd", workspace_label: "proj" });
    expect(sv.path).toBe("/etc/passwd");
    expect(sv.workspace_label).toBe("proj");
  });
  ...
  it("isAppError() is true for every variant instance", () => {
    const all: AppError[] = [
      ...
      new ToolCall({ tool_call_id: "t", message: "a" }),
      new SandboxViolation({ path: "/p", workspace_label: "w" }),
