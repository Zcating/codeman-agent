...
import { XCircle, CheckCircle2, Zap } from "lucide-solid";
...
export function MessageBubble(props: { message: Message }) {
  ...
  const isCompactionSummary = createMemo(() => {
    return props.message.mode === "compaction" && props.message.summary === true;
  });
      ...
      <div class={`mb-3 flex w-full ${role() === "user" ? "justify-end" : "justify-start"}`}>
        ...
        <Show when={role() === "assistant"}>
          <div
            ...
            data-message-mode={isCompactionSummary() ? "compaction" : undefined}
          >
            <Show when={isCompactionSummary()}>
              <div
                class="flex items-center gap-1.5 text-xs font-medium text-warning"
                data-testid="compaction-summary-label"
              >
                <Zap class="h-3 w-3" aria-hidden="true" />
                <span>上下文已压缩</span>
              </div>
