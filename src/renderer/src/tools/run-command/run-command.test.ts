...
describe("runCommandTool", () => {
  beforeEach(() => {
    ...
    mockState.resolvedByCommand = {};
  });

  it("formats successful command result correctly", async () => {
    mockState.resolvedByCommand = {
      runCommandAssess: { risk: { kind: "low", reasons: [] }, requestID: undefined },
      runCommandExecute: { status: "ok", exitCode: 0, stdout: "hi\n", stderr: "", durationMs: 100 },
    };
  ...
  it("formats error result correctly", async () => {
    mockState.resolvedByCommand = {
      runCommandAssess: { risk: { kind: "low", reasons: [] }, requestID: undefined },
      runCommandExecute: { status: "error", error: { kind: "PermissionDenied", message: "denied" } },
    };
  ...
  it("formats cancelled result with partial stdout", async () => {
    mockState.resolvedByCommand = {
      runCommandAssess: { risk: { kind: "low", reasons: [] }, requestID: undefined },
      runCommandExecute: { status: "cancelled", partialOutput: { stdout: "partial\nstdout\n", stderr: "" } },
    };
  ...
  it("formats timeout result with partial stderr", async () => {
    mockState.resolvedByCommand = {
      runCommandAssess: { risk: { kind: "low", reasons: [] }, requestID: undefined },
      runCommandExecute: { status: "timeout", partialOutput: { stdout: "", stderr: "Timed out after 30s\n" } },
    };
  ...
  it("passes command/cwd/timeoutMs via IPC invoke", async () => {
    mockState.resolvedByCommand = {
      runCommandAssess: { risk: { kind: "low", reasons: [] }, requestID: undefined },
      runCommandExecute: { status: "ok", exitCode: 0, stdout: "", stderr: "", durationMs: 0 },
    };
    await runCommandTool.execute("t3", { command: "git status", cwd: "/work", timeoutMs: 1000 });
    const call = mockState.invokeCalls.find((c) => c.name === "runCommandExecute");
