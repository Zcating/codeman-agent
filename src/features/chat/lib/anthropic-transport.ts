...
export class AnthropicTransport {
  ...
  async *run(
    ...
    // Yielded in the final agent_end event so pi-agent.prompt() adds them to _state.messages.
    ...
    try {
      while (turnIndex < MAX_TURNS) {
        ...
        // Single LLM turn, yields message_start/update/end internally.
        const turnGen = this.streamTurn(apiKey, baseUrl, cfg.model, systemPrompt, currentMessages, tools, signal);
        let assistantMsg: AssistantMsgLike | undefined;
        while (true) {
          const { value, done } = (await turnGen.next()) as IteratorResult<unknown, AssistantMsgLike>;
          if (done) {
            assistantMsg = value;
            break;
          }
        ...
        // Append assistant to currentMessages (next LLM sees it) and generatedMessages.
        ...
        // No tool calls -> terminate loop.
        if (toolCalls.length === 0) {
          yield { type: `turn_end`, message: generatedMessages[generatedMessages.length - 1], toolResults: [] };
          break;
        }

        // Execute each tool call, emitting 4 events per call.
        const toolResultMessages: any[] = [];
        for (const tc of toolCalls) {
          const tool = tools?.find((t: any) => t.name === tc.name) as
            | { name: string; execute: (id: string, args: unknown, signal?: AbortSignal) => Promise<any> }
            | undefined;
          ...
          // Normalize to ToolResultMessage format (role: toolResult).
      ...
      if (turnIndex >= MAX_TURNS) {
        // Defensive: LLM dead-loop calling the same tool. Truncate the loop.
        console.warn(`[AnthropicTransport] reached MAX_TURNS=${MAX_TURNS}, terminating agent loop`);
      }
    } finally {
      // Always yield agent_end so pi-agent can resolve its running prompt.
      // Without this, chat-view setRunning(false) never runs and UI freezes.
