...
describe("ChatView", () => {
  ...
  it("tool 角色的消息渲染工具结果", () => {
    ...
    // 验证工具结果标签存在
    const hasToolResult = Array.from(details).some((d) =>
      d.textContent?.includes("工具结果")
    );
  ...
  it("tool 消息显示工具调用 ID 和结果", () => {
    ...
    const hasToolCallId = Array.from(codeElements).some(
      (code) => code.textContent === "tc-read-1"
    );
