...
describe("ChatView", () => {
  ...
  // ─── ThinkingPanel 集成测试 (V2.x: 独立 panel,stream 中和完后都显示) ─────
  it("ThinkingPanel: streaming + stub.thinking 非空 → 渲染(open) + 含实际 thinking 文本", async () => {
    const conversationsStoreMock = await import("../stores/chat.store");
    const mockStore = (conversationsStoreMock as unknown as { store: { byId: Record<string, { streamingMessageId: string | null; messages: Message[] }> } }).store;
    mockStore.byId["conv-1"].streamingMessageId = "msg-stub";
    mockStore.byId["conv-1"].messages = [
      ...
      },
      {
        ...
        role: "assistant",
        content: "",
        thinking: "verifying file paths before edit...",
        toolCalls: null,
        toolResults: null,
    ...
    expect(panel?.getAttribute("data-message-id")).toBe("msg-stub");
    // panel 本身是 <details> 根元素 — streaming=true 时 open attr 出现
    expect(panel?.hasAttribute("open")).toBe(true);
    expect(panel?.textContent).toContain("正在思考…");
    expect(panel?.textContent).toContain("verifying file paths before edit...");
  });

  it("ThinkingPanel: done(streaming=null) + 最后一条 assistant.thinking 非空 → 持续显示(closed,可手动展开)", async () => {
    const conversationsStoreMock = await import("../stores/chat.store");
    const mockStore = (conversationsStoreMock as unknown as { store: { byId: Record<string, { streamingMessageId: string | null; messages: Message[] }> } }).store;
    mockStore.byId["conv-1"].streamingMessageId = null;
    mockStore.byId["conv-1"].messages = [
      {
        ...
        thinking: "decided after careful review.",
        toolCalls: null,
        toolResults: null,
        model: "MiniMax-M2.5-highspeed",
        inputTokens: null,
    ...
    const panel = container.querySelector('[data-testid="thinking-panel"]');
    expect(panel).toBeTruthy();
    // streaming=false → <details> 默认折叠 (panel 本身是 <details> 根元素)
    expect(panel?.hasAttribute("open")).toBe(false);
    // 总结文字反映 done 状态
    expect(panel?.textContent).toContain("已思考");
    // 已累积的 thinking 文本仍在 panel <pre> 里
    expect(panel?.textContent).toContain("decided after careful review.");
  });

  it("ThinkingPanel: 没任何 assistant.thinking → 不渲染", async () => {
    const conversationsStoreMock = await import("../stores/chat.store");
    const mockStore = (conversationsStoreMock as unknown as { store: { byId: Record<string, { streamingMessageId: string | null; messages: Message[] }> } }).store;
    mockStore.byId["conv-1"].streamingMessageId = null;
    mockStore.byId["conv-1"].messages = [
