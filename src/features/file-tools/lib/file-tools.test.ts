...
describe("readFileTool", () => {
  ...
  it("error path returns SandboxViolation as error string", async () => {
    mockState.rejected = { kind: "SandboxViolation", path: "/etc/x", workspace_label: "ws1" } as unknown as Error;
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("SandboxViolation") });
    expect(result.details).toMatchObject({ kind: "SandboxViolation" });
  });

  it("error path returns NotFound as error string", async () => {
    mockState.rejected = { kind: "NotFound", message: "File not found: /tmp/nonexistent.txt" } as unknown as Error;
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("File not found") });
...
describe("editFileTool", () => {
  ...
  it("happy path unique replace returns success", async () => {
    ...
    expect(mockState.invokeCalls.find((c) => c.name === "edit_file")).toMatchObject({
      args: { workspace_id: "ws1", path: "/tmp/x.txt", old_text: "foo", new_text: "bar", replace_all: false },
    });
  });
  ...
  it("replace_all returns correct message", async () => {
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("all occurrences replaced") });
  });

  it("error path when old_text matches multiple times", async () => {
    mockState.rejected = { kind: "Unknown", message: "old_text must match exactly once (got 2)" } as unknown as Error;
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("exactly once") });
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("got 2") });
  });
});
...
describe("searchFilesTool", () => {
  ...
  it("happy path returns matches with line numbers", async () => {
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("Found 2 match(es)") });
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("src/main.ts:10") });
  ...
  it("happy path returns empty array", async () => {
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("No matches found") });
  });

  it("error path returns SandboxViolation", async () => {
    mockState.rejected = { kind: "SandboxViolation", path: "/etc", workspace_label: "ws1" } as unknown as Error;
...
describe("deleteFileTool", () => {
  ...
  it("happy path returns void with success message", async () => {
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("recycle bin") });
  ...
  it("error path returns SandboxViolation", async () => {
    mockState.rejected = { kind: "SandboxViolation", path: "/etc/passwd", workspace_label: "ws1" } as unknown as Error;
  ...
  it("error path for blocked extension returns clear error", async () => {
    mockState.rejected = { kind: "Unknown", message: "Blocked file type: .exe" } as unknown as Error;
    ...
    expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("Blocked file type: .exe") });
