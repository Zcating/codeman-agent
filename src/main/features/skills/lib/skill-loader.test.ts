 ...
 *
 * 测试场景（与 PR-δ 前一致）：
 * - parseFrontmatter：6 个 frontmatter 解析 case
 * - scanSkillsDir：空目录 / 不存在目录 / 单 skill / 多 skills / corrupt frontmatter 跳过 /
 *   缺 description 跳过 / 非目录条目跳过 / preinstalled 路径识别
 * - loadSkillContent：读完整内容 / 不存在返回 NotFound / 空 body 仍返回完整内容
 */
...
describe("scanSkillsDir", () => {
  it("空目录返回空数组", async () => {
    const result = await runWithFs(scanSkillsDir(tempDir));
    expect(result).toEqual([]);
  });

  it("不存在的目录返回空数组", async () => {
    const result = await runWithFs(
      scanSkillsDir(join(tempDir, "nonexistent")),
    );
    expect(result).toEqual([]);
  });
  ...
  it("preinstalled 路径识别(source = preinstalled)", async () => {
    ...
    const result = await runWithFs(scanSkillsDir(preinstalledRoot));
    expect(result).toHaveLength(1);
...
describe("loadSkillContent", () => {
  it("读取完整 skill 内容", async () => {
    ...
    const result = await runWithFs(loadSkillContent(tempDir, "foo"));
    expect(result).toBe(content);
  });

  it("skill 不存在返回 AppBackendError.NotFound", async () => {
    const exit = await Effect.runPromiseExit(
      loadSkillContent(tempDir, "missing").pipe(Effect.provide(TestLayer)),
    );
    expect(Exit.isFailure(exit)).toBe(true);
    if (Exit.isFailure(exit)) {
      const error = (
        exit.cause as { _tag: "Fail"; error: unknown }
      ).error;
      expect(error).toBeInstanceOf(AppBackendError.NotFound);
    }
  });

  it("空 body 的 skill 仍返回完整内容(含 frontmatter)", async () => {
    await makeSkill("empty", "---\nname: empty\ndescription: E\n---\n");
    const result = await runWithFs(loadSkillContent(tempDir, "empty"));
