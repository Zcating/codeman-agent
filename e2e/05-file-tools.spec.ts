...
test.describe("05 — 文件工具 (mock LLM)", () => {
  ...
  test.afterAll(async () => {
    ...
    try { fs.rmSync(e2eRoot, { recursive: true, force: true }); } catch {}
  });
  ...
  test("edit_file — old_text 匹配多次时报错,精确匹配时成功", async () => {
    ...
    await enqueueMockResponse(page, {
      toolCalls: [
        {
          name: "edit_file",
          input: { workspaceId: "main", path: "target.txt", oldText: "TODO", newText: "TASK", replaceAll: false },
        },
      ],
    });
    await enqueueMockResponse(page, {
    ...
    while (Date.now() < deadline1) {
      const assistantBubbles = await page.locator("div.justify-start > div[class*='bg-card']").count();
      if (assistantBubbles > 0) { sawResult = true; break; }
  ...
  test("search_files 返回匹配文件 + 行号,不包含无关文件", async () => {
    ...
    await enqueueMockResponse(page, {
      toolCalls: [
        { name: "search_files", input: { workspaceId: "main", glob: "**/*.ts", contentPattern: "TODO" } },
      ],
    });
    await enqueueMockResponse(page, {
    ...
    const bodyText = await page.evaluate(() => document.body.textContent);
    expect(bodyText?.includes("a.ts"), "结果应包含 a.ts,实际: " + bodyText?.slice(0, 300)).toBe(true);
    expect(
