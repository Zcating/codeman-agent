...
import { assert, clearAllHistory, disposeTauriPage, getTauriPage, invoke, submitForm } from "./helpers";
...
test.describe("03 — 聊天 → billing 工具", () => {
  ...
  test("发送消息并验证聊天循环活着", async () => {
    ...
    while (Date.now() < deadline) {
      ...
      const assistantBubbles = await page.locator(
      ...
      const diagState = await page.evaluate(() => {
        ...
        return {
          ...
          hasCancelBtn: !!Array.from(document.querySelectorAll('button')).find(b => /cancel/i.test(b.textContent ?? '')),
        };
      });
