...
import { CDP_INJECT_SCRIPT } from "./cdp-driver";
...
export async function resetChatState(): Promise<void> {
  ...
  await assert.visible(p.locator('textarea[placeholder="Type a message…"]'), {
    timeout: 15_000,
  });
...
export async function clickNewConversationAndWait(
  ...
  await p.locator('button[title="New conversation"]').click();
  const activeItem = p.locator("aside li.bg-primary-500").first();
