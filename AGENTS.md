...
## 目录布局 (Feature-Sliced — 2026-06-14)
...
│   ├── shared/                    # 跨 feature 共享（按 type 分类，不是按 feature）
│   │   ├── lib/                   # tauri.ts (IPC 入口) + cn.ts (className 合并)
│   │   ├── types/                 # 跨域类型 (Settings/Message/Snapshot/AppError)
│   │   ├── state/                 # 跨域 Solid 状态 (theme)
│   │   ├── assets/                # logo.svg 等
│   │   ├── ui/                    # 5 原子: Button/Input/Textarea/Checkbox/Card
│   │   └── AGENTS.md              # shared 规则
│   └── features/
│       ├── chat/                  # 聊天域 — runtime + 2 stores + 4 components + routes
│       │   ├── components/        # Sidebar / ChatView / MessageBubble / ToolCallCard
│       │   ├── store/             # conversations + messages（Effect→Solid 桥接层）
│       │   ├── runtime.ts         # AgentRuntime (Stream wrapper for pi-mono)
│       │   ├── routes/            # / 路由
│       │   ├── index.ts           # public API barrel
│       │   └── AGENTS.md
│       ├── settings/              # 设置域 — ProviderCard + 2 subsystems + routes
│       │   ├── components/        # ProviderCard (用 Card 7 子件重构)
│       │   ├── subsystems/        # llm_providers + system_prompt
│       │   ├── routes/            # /settings 路由
│       │   ├── index.ts
│       │   └── AGENTS.md
│       └── billing/               # 工具域 — 仅 tools + types（无 UI）
│           ├── tools/             # get_balance + get_plan_quota (LLM 工具 schema)
│           ├── index.ts
│           └── AGENTS.md
│
